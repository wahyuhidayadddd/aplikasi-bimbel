...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card text-center">
            ...
            <div class="stat-value text-primary" id="totalSiswa">-</div>
    ...
    <div class="col-6 col-md-3">
        <div class="stat-card text-center">
            ...
            <div class="stat-value text-success" id="totalAktif">-</div>
...
<div class="stat-card">
    ...
    <div class="row g-2 mb-3">
        ...
        <div class="col-md-3">
            <select id="filterStatus" class="form-select form-select-sm" onchange="loadStudents()">
        ...
        <div class="col-md-3">
            <select id="filterBranch" class="form-select form-select-sm" onchange="loadStudents()">
        ...
        <div class="col-md-2">
            <button class="btn btn-sm btn-outline-secondary w-100" onclick="loadStudents()">
...
function loadStudents(page = 1) {
    ...
    $.get('{{ route("admin.students.index") }}', {
        ...
        // Update stats
        $('#totalSiswa').text(res.total ?? 0);
        $('#totalAktif').text(res.data?.filter(s => s.status === 'aktif').length ?? 0);
        $('#totalL').text(res.data?.filter(s => s.gender === 'L').length ?? 0);
        $('#totalP').text(res.data?.filter(s => s.gender === 'P').length ?? 0);
        ...
        if (!res.data || res.data.length === 0) {
            ...
            res.data.forEach((s, i) => {
                const statusBadge = {
                    aktif: '<span class="badge rounded-pill" style="background:#dcfce7;color:#15803d">● Aktif</span>',
                    nonaktif: '<span class="badge rounded-pill" style="background:#f3f4f6;color:#6b7280">● Nonaktif</span>',
                    lulus: '<span class="badge rounded-pill" style="background:#dbeafe;color:#1d4ed8">● Lulus</span>',
                }[s.status] ?? '<span class="badge bg-secondary">-</span>';

                const avatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(s.name)}&background=${s.gender === 'P' ? 'ec4899' : '4f8ef7'}&color=fff&size=40`;
                ...
                html += `<tr>
                    ...
                    <td class="text-center">
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-info" onclick="showDetail(${s.id})" title="Detail">
                                <i class="bi bi-eye"></i>
                            </button>
                            <button class="btn btn-outline-warning" onclick="editStudent(${s.id})" title="Edit">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button class="btn btn-outline-danger" onclick="deleteStudent(${s.id})" title="Hapus">
        ...
        // Pagination info
        $('#paginationInfo').text(`Menampilkan ${res.from ?? 0}-${res.to ?? 0} dari ${res.total ?? 0} siswa`);

        // Pagination buttons
        let pages = '';
        if (res.last_page > 1) {
            pages += `<button class="btn btn-sm ${res.current_page == 1 ? 'btn-secondary disabled' : 'btn-outline-secondary'} me-1"
                onclick="loadStudents(${res.current_page - 1})"><i class="bi bi-chevron-left"></i></button>`;
            for (let p = 1; p <= res.last_page; p++) {
                if (p == 1 || p == res.last_page || Math.abs(p - res.current_page) <= 1) {
                    pages += `<button class="btn btn-sm ${p == res.current_page ? 'btn-primary' : 'btn-outline-secondary'} me-1"
                        onclick="loadStudents(${p})">${p}</button>`;
                } else if (Math.abs(p - res.current_page) == 2) {
                    pages += `<span class="me-1">...</span>`;
                }
            }
            pages += `<button class="btn btn-sm ${res.current_page == res.last_page ? 'btn-secondary disabled' : 'btn-outline-secondary'}"
                onclick="loadStudents(${res.current_page + 1})"><i class="bi bi-chevron-right"></i></button>`;
        }
        $('#paginationLinks').html(pages);
    });
}

function openModal(title = 'Tambah Siswa') {
    $('#modalTitle').html(`<i class="bi bi-person-plus me-2"></i>${title}`);
    $('#studentForm')[0].reset();
    $('#studentId').val('');
...
function editStudent(id) {
    $.get(`/admin/students/${id}`, function(res) {
        ...
        $('#nis').val(s.nis);
        $('#gender').val(s.gender);
        $('#birth_date').val(s.birth_date);
        $('#birth_place').val(s.birth_place);
        $('#branch_id').val(s.branch_id);
        $('#school_name').val(s.school_name);
        $('#grade').val(s.grade);
        $('#phone').val(s.phone);
        $('#parent_name').val(s.parent_name);
        $('#parent_phone').val(s.parent_phone);
...
function saveStudent() {
    ...
    $.ajax({
        ...
        error(xhr) {
            ...
            let msg = errors ? Object.values(errors).flat().join('<br>') : (xhr.responseJSON?.message ?? 'Terjadi kesalahan');
...
// Search realtime
let timer;
$('#searchInput').on('input', () => { clearTimeout(timer); timer = setTimeout(loadStudents, 400); });
